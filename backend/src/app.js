...
app.post('/api/invite/expire', async (req, res) => {
  ...
  await prisma.inviteCode.update({ where: { code }, data: { status: 'expired' } });
...
app.post('/api/invite/restore', async (req, res) => {
  ...
  await prisma.inviteCode.update({ where: { code }, data: { status: 'unused', usedByPhone: null, usedAt: null } });
  res.json({ success: true });
});
